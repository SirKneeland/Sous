...
def main():
    ...
    # Every palette value must live in the theme. TexturePreviewView builds colors
    # from live slider values for texture tuning and is deliberately exempt.
    exempt = {"SousTheme.swift", "TexturePreviewView.swift"}
    for swift in (ROOT / "ios" / "SousApp" / "SousApp").rglob("*.swift"):
        if swift.name in exempt:
            continue
    ...
    print(f"OK — {checked} tokens match SousTheme.swift, no hardcoded colors in views.")
    return 0
